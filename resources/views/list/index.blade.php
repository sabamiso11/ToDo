...
@section('content')
  ...
  @forelse ($lists as $list)
    <div class="card mb-2">
      <div class="card-body">
        <form method="post" action="{{ action('TaskListController@destroy', $list) }}" >
          {{ csrf_field() }}
          <button type="submit" class="btn btn-outline-secondary btn-sm position-absolute" id="destroy" name="destroy" value="{{ $list }}">×</button>
        </form>
